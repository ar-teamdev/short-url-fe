...
RUN npm set progress=false && npm config set depth 0 \
	&& npm install --no-install-recommends --legacy-peer-deps \
	&& mkdir /app \
...
RUN node --max_old_space_size=4096 ./node_modules/@angular/cli/bin/ng build --build-optimizer=false --output-path=dist --configuration=development
